...
void face_landmark_detection(dlib::array2d<unsigned char>&, dlib::shape_predictor, std::vector<cv::Point2f>&);
lint get_time();
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        std::cerr << "Usage: <Program Name> <Source Image> <Pattern Image> <Output Name>" << std::endl;
        return -1;
    }
    ...
    dlib::load_image(img_dlib1, argv[1]);
    dlib::load_image(img_dlib2, argv[2]);
    cv::Mat img_cv1 = cv::imread(argv[1]);
    cv::Mat img_cv2 = cv::imread(argv[2]);
	...
	face_landmark_detection(img_dlib2, sp, points2);
    std::cerr << "Face landmarks generated." << std::endl;
    current_time = calc_time_elapsed(current_time);
    ...
    std::cerr << "Convex hull found." << std::endl;
    current_time = calc_time_elapsed(current_time);
    ...
    std::cerr << "Delaunay triangulation finished." << std::endl;
    current_time = calc_time_elapsed(current_time);
    ...
    std::cerr << "Clone finished." << std::endl;
    current_time = calc_time_elapsed(current_time);
...
void delaunay_triangulation(const std::vector<cv::Point2f>& hull, std::vector<correspondens>& delaunay_tri, cv::Rect rect) {
	...
	for (const auto& i : hull) subdiv.insert(i);
